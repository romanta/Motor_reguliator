...
int min_motor_spd = 1000;  //Минимальное значени ШИМ 0.8 мс
...
int Powpin = A0;  // analog pin used to connect the potentiometer
...
const int reed = 3; //задаем ножку процессора для подключения счетчика на герконе
...
int delta_WH;//буфер расчетов значений для ШИМ
...
void setup() {
  ...
  circumference = (2 * 3.415926535 * radius) / 1000000;
  // Для 2 счетчика
  pinMode(reed2, INPUT);
  reedCounter2 = maxReedCounter2;
  circumference2 = (2 * 3.415926535 * radius2) / 1000000;
...
/* Тут надо подумать, чтоб не повторять предыдущий блок, а интегрировать временную задержку каденса в блок подсчета для колеса.
  В целом надо организовать условия при которых всегда будет нулевая скорость если - колесо не крутится или педеали не крутятся.
*/
ISR(TIMER4_COMPA_vect) {// Прерывание при частоте 1 кГц для измерения геркона
  ...
  if (timer_kd > 2000) { // Если время накапало больше 2000 мс
    ...
    delta_KD = timer_kd; // то значение дельты тоже 2000, дабы избежать зависших значений дельты, при отстановке педалей
  }
  else {
  ...
  // Тут кусок кода для мигания светодиодом, для проверки настройки датчиков
  if (reedVal_2 == HIGH) { //
    // turn LED on://
...
ISR(TIMER3_COMPA_vect) {// Прерывание при частоте 1 кГц для измерения геркона
 ...
 //   motor_spd = min_motor_spd; // Если таймер больше 2000 то пишем в порт мотора нулевую скорость.
...
//  }
  ...
  // Power = analogRead(Powpin);            // считывает значение потенциометра (значение от 0 до 1023)
  ...
  // тут пробуем выполнить условия ступенчатого регулирования скорости, в зависимости от показаний задержки времени.
  // своеобразная КОРОБКА ПЕРЕДАЧ

  if (delta_KD > 950 || delta_WH > 520 || delta_WH < 130 ) {//если значение таймера  больше "" или  больше ""
    motor_spd = 1000;//то пишем в порт мотора нулевую скорость.
...
//    motor.write(motor_spd);
 ...
 if (delta_KD < 950 || delta_WH < 200 || delta_WH > 150 ) { //если значение таймера  больше "" или  больше ""
    motor_spd = 1858;//то пишем в порт мотора @@ скорость.
 //   motor.write(motor_spd);
    start_motor = 14;
  }
 if (delta_KD < 950 || delta_WH < 250 || delta_WH > 200 ) { //если значение таймера  больше "" или  больше ""
    motor_spd = 1792;//то пишем в порт мотора @@ скорость.
 //   motor.write(motor_spd);
    start_motor = 13;
  }
  if (delta_KD < 950 || delta_WH < 300 || delta_WH > 250 ) { //если значение таймера  больше "" или  больше ""
    motor_spd = 1726;//то пишем в порт мотора @@ скорость.
//    motor.write(motor_spd);
    start_motor = 12;
  }
 if (delta_KD < 950 || delta_WH < 350 || delta_WH > 300 ) { //если значение таймера  больше "" или  больше ""
    motor_spd = 1660;//то пишем в порт мотора @@ скорость.
//    motor.write(motor_spd);
    start_motor = 11;
  }
  if (delta_KD < 950 || delta_WH < 400 || delta_WH > 350 ) { //если значение таймера  больше "" или  больше ""
    motor_spd = 1080;//то пишем в порт мотора @@ скорость.
//    motor.write(motor_spd);
    start_motor = 10;
  }
 if (delta_KD < 950 || delta_WH < 450 || delta_WH > 400 ) { //если значение таймера  больше "" или  больше ""
    motor_spd = 1594;//то пишем в порт мотора @@ скорость.
//    motor.write(motor_spd);
    start_motor = 9;
  }
  if (delta_KD < 950 || delta_WH < 500 || delta_WH > 450 ) { //если значение таймера  больше "" или  больше ""
    motor_spd = 1528;//то пишем в порт мотора @@ скорость.
//    motor.write(motor_spd);
    start_motor = 8;
  }
  if (delta_KD < 950 || delta_WH < 650 || delta_WH > 500 ) { //если значение таймера  больше "" или  больше ""
    motor_spd = 1462;//то пишем в порт мотора @@ скорость.
 //   motor.write(motor_spd);
    start_motor = 7;
  }
   if (delta_KD < 950 || delta_WH < 700 || delta_WH > 650 ) { //если значение таймера  больше "" или  больше ""
    motor_spd = 1396;//то пишем в порт мотора @@ скорость.
 //   motor.write(motor_spd);
    start_motor = 6;
  }
 if (delta_KD < 950 || delta_WH < 750 || delta_WH > 700 ) { //если значение таймера  больше "" или  больше ""
    motor_spd = 1330;//то пишем в порт мотора @@ скорость.
 //   motor.write(motor_spd);
    start_motor = 5;
  }
if (delta_KD < 950 || delta_WH < 800 || delta_WH > 750 ) { //если значение таймера  больше "" или  больше ""
    motor_spd = 1264;//то пишем в порт мотора @@ скорость.
//    motor.write(motor_spd);
    start_motor = 4;
  }
  if (delta_KD < 950 || delta_WH < 850 || delta_WH > 800 ) { //если значение таймера  больше "" или  больше ""
    motor_spd = 1198;//то пишем в порт мотора @@ скорость.
 //   motor.write(motor_spd);
    start_motor = 3;
  }
 if (delta_KD < 950 || delta_WH < 900 || delta_WH > 850 ) { //если значение таймера  больше "" или  больше ""
    motor_spd = 1132;//то пишем в порт мотора @@ скорость.
 //   motor.write(motor_spd);
    start_motor = 2;
  }
if (delta_KD < 950 || delta_WH < 950 || delta_WH > 900 ) { //если значение таймера  больше "" или  больше ""
    motor_spd = 1066;//то пишем в порт мотора @@ скорость.
//    motor.write(motor_spd);
    start_motor = 1;
  }
...
//  Serial.print(timer_wh);  Serial.println(" timer_wh");// после отладки отключить
  ...
  Serial.print(motor_spd);  Serial.println(" motor");// после отладки отключить
  delay(1000);
}
